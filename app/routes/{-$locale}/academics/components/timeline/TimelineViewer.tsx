...
export default function TimelineViewer<T extends ProcessedTimelineContent>({
  ...
  if ((contents[0]?.year ?? 0) !== prevFirstYear) {
    ...
    setSelectedYear(contents[0]?.year ?? 0);
  }
  ...
  // TanStack Link `to`는 라우트 패턴 유니온을 기대 → 템플릿 리터럴 타입을 string으로 넓혀 전달.
  const createPath: string = `${pathname}/create`;
    ...
    <>
      <LoginVisible allow="ROLE_STAFF">
        <div className="mt-7 flex justify-end gap-3">
          ...
          <Button variant="secondary" as="link" to={`${pathname}/edit/${year}`}>
            {t('편집')}
          </Button>
        </div>
      </LoginVisible>
