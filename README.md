...
apps/api/        백엔드 (Kotlin/Spring). 옛 csereal-server — 자체 README 참고
e2e/             Playwright E2E
infra/           compose 스택(로컬·prod)·Caddy·모니터링·운영 스크립트·배포 대상
...
`pnpm test`(E2E)는 **로컬 docker 백엔드**가 필요합니다(`pnpm web:dev`는 staging 백엔드를 보므로 불필요). 백엔드는 같은 커밋의 `apps/api` 소스에서 `infra/compose.yml`+`compose.local.yml` 로 자동 기동됩니다 — 별도 체크아웃이 없습니다. 비주얼 baseline 도 그 백엔드 기준입니다.

백엔드 단위 테스트는 `pnpm api:test`.
...
- [wafflestudio/csereal-server](https://github.com/wafflestudio/csereal-server) — 2026-09 이 레포의 `apps/api` 로 합쳐졌습니다(히스토리 보존). 아카이브 예정.
