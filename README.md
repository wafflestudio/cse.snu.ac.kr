...
flowchart TD
  ...
  feat -. "PR마다" .-> ci
  dev -. "PR마다" .-> ci
  ci["ci.yml<br/>web-test(typecheck·lint·knip + E2E) · api-test"]
...
- **PR 게이트(`ci.yml`):** `web-test`(타입/린트/knip + E2E, 같은 커밋의 `apps/api` 로 백엔드 기동)가 통과해야 머지. `apps/api` 가 바뀐 PR 은 `api-test`(Gradle)도 돈다.
