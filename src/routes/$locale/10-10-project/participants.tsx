...
function TenTenParticipantsPage() {
...
export const Route = createFileRoute('/$locale/10-10-project/participants')({
  loader: async ({ params }) => {
    const locale = params.locale === 'en' ? 'en' : 'ko';
    // 이 경로는 본문에 가시 링크 텍스트로 노출된다(baseline). 클릭 시 서버가 /{lang}으로 리다이렉트.
