...
- **단일 선택은 Button 토글이 아니라 네이티브 radiogroup.** 상호배타 선택(세그먼트·카테고리 필터)은 `aria-pressed` 토글 버튼이 아니라 `fieldset`+native `radio`(pill 스타일)로 — 그룹 시맨틱·화살표 키 이동을 브라우저가 준다(faculty 정렬·공지 필터). 그 결과 Button `kind`은 **상태 없는 5개**(primary/action/secondary/quiet/nav)만 남았다(toggle·segmented 제거, 사용처 1곳이던 link는 Tag 내부에 직접 정의).
