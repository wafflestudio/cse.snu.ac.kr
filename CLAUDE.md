...
- **프론트 = 프로덕션 빌드**를 `server.ts`(Hono)로 서빙. MSW/mock 안 씀. **왜 same-origin proxy:** 세션 쿠키(JSESSIONID)가 `Secure` 라 cross-origin 요청에 안 실린다 → 브라우저는 :3000 만 보고 `/api` 를 서버사이드에서 :8080 으로 넘긴다. 배포는 엣지가 `/api` 를 백엔드로 넘기므로 프록시가 없다(`API_PROXY_TARGET` 없음).
- **SSR 이 부를 백엔드는 런타임 env `API_ORIGIN` 이다**(빌드에 박지 않는다). 브라우저는 언제나 same-origin `/api`. 한 이미지가 어느 환경에서도 뜨고, 새 환경은 env 만 주면 된다.
...
인터넷 ─443─> Caddy(엣지: TLS·HSTS·HTTP/2·라우팅·보안 헤더, 별도 compose ~/proxy)
   ...
   web SSR 은 엣지를 돌지 않고 백엔드 컨테이너를 직접 부른다(`API_ORIGIN=http://api:8080`).
```
