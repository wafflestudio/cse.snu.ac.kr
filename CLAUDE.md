...
- **백엔드: 로컬 docker 실서버** (`../csereal-server-main`, local 프로파일, :8080).
  ...
  **로컬 전용(격리·리셋 자유)이라 리셋+시드 자유. staging·프로덕션 서버는 절대 건드리지 않는다.**
  기동: `docker compose -f ../csereal-server-main/docker-compose-local-full.yml -f tests/setup/backend/docker-compose-fe-test.yml up -d` (Playwright가 자동).
...
> 고정해야 baseline이 호스트(macOS 등) 무관하게 픽셀 동일하기 때문.
> **솔로 레포라 CI는 없다 — 로컬 `pnpm test`가 단일 게이트다.**
...
`e2e-docker.sh` 동작: ① 백엔드(MySQL+Spring)를 **호스트 docker의 main 체크아웃(`../csereal-server-main`)
compose로 보장**(프로젝트명 `csereal-server-main` 고정 → 떠 있으면 no-op·없으면 기동; "떠 있는 :8080
아무거나 재사용"을 안 해 어느 백엔드인지 모호함이 없다 — v3 등이 :8080 점유 중이면 포트 충돌로 즉시 드러남),
...
### 비주얼 baseline은 Linux 단일 (컨테이너 정본)

> baseline은 **`*-linux.png` 단일 세트**. 핀된 컨테이너가 정본 렌더 환경이라 머신이 달라도 픽셀 동일.

- **백엔드 기준 = `../csereal-server-main`(origin/main).** baseline은 이 main 백엔드에서 찍는다.
  E2E가 main 백엔드의 스모크 검증도 겸한다.
  - 트레이드오프: main이 floating이라 **백엔드가 바뀌면 FE를 안 건드렸는데도 baseline이 깨질 수 있다**
    → 그땐 백엔드를 최신 main으로 올리고(§백엔드 버전 동기화) baseline을 재생성한다.
- **baseline 재생성은 main 백엔드에서.** `e2e-docker.sh`가 항상 `../csereal-server-main`으로 백엔드를
  띄우므로 로컬에서 `pnpm test --update-snapshots`로 안전하게 생성한다(CI는 제거됨 — 솔로 레포라 로컬이
  단일 게이트). v3 등 다른 백엔드가 :8080을 점유 중이면 compose가 충돌로 막아주니, 내리고 다시 돌린다.

## 백엔드 버전 동기화 (중요)

테스트용 백엔드는 **`../csereal-server-main`**(main 전용 체크아웃, origin/main 유지). docker가 **소스의
prebuilt JAR를 COPY**한다(Dockerfile) → 소스가 낡으면 docker도 낡는다. 뒤처졌으면 최신 main으로 올린다:
```bash
...
```
올린 뒤 `pnpm test --update-snapshots`로 baseline을 재생성한다. (v3 등 다른 작업본은 형제 `../csereal-server`에 둔다.)
...
4. **baseline** — `pnpm test --update-snapshots`(핀된 컨테이너), PNG 커밋(`-read-linux.png` + `-read-mobile-linux.png`). 백엔드는 e2e-docker.sh가 항상 `../csereal-server-main`(main)으로 띄운다(위 §실행).
...
- **비주얼 baseline은 Linux 단일**(`*-linux.png`). 핀된 Playwright 컨테이너가 정본 렌더 환경이라 머신 무관하게 픽셀 동일(위 §실행 "비주얼 baseline은 Linux 단일" 참고).
...
- **설정:** SB 10.4 `@storybook/tanstack-react`(TanStack Start 자동감지·라우터 컨텍스트 내장). 스토리 글롭 `app/**/*.stories.@(ts|tsx)`(컴포넌트 코로케이션). `.storybook/`: `main.ts`(`defineMain`; viteFinal에 `@tailwindcss/vite` + `@/lib/serverFns` alias)·`preview.tsx`(`definePreview`; app.css·sonner import + autodocs + `addons:[addonA11y(), addonDocs()]`)·`withForm.tsx`(RHF FormProvider 데코). Chromatic 애드온은 제거(미사용 — 픽셀 회귀는 E2E 담당). (CI 없음 — 스토리 빌드 회귀는 로컬 `pnpm build-storybook`/배포 빌드에서만 드러난다.)
...
- **함정(재발 주의):**
  ...
  - **union 필수 prop은 스토리에 args 명시** — string-literal union 필수 prop(`variant`/`colorTheme`/`theme`/`selected` 등)을 가진 컴포넌트는 CSF4가 `meta.args`만으론 "충족"을 인식 못 한다(한 prop이라도 union이면 `children` 포함 **필수 args 전체**를 각 스토리에서 재요구) → 해당 스토리에 args를 명시(공통 `children`은 상수로 공유). discriminated-union 컴포넌트의 합성 render는 무인자(`render: () => …`)로 둬 union 파라미터 추론을 피한다. (union 없는 필수 prop은 `meta.args`로 충족됨 — 예: Fieldset은 필수 `children`/`title` 있어도 bare `meta.story()` OK.) typed parameters라 `docs.story.iframeHeight`는 number 아닌 **string**(`'440px'`).
...
- **스토리 작성 원칙(사용자 지시):**
  ...
  2. **DS에 우겨넣지 않는다.** 컴포넌트 일관성이 깨지는 사용처는 비주얼이 깨지더라도 **앱 코드를 고친다**(컴포넌트 API를 늘려 수용 X). 이렇게 잡은 실버그: Tag 삭제버튼 `aria-label`→`ariaLabel`(접근명 누락).
  3. **편집 불가 prop은 컨트롤로 욱여넣지 않는다.** 함수/복합 union(예: Calendar `disabled`=DayPicker Matcher)·ReactNode는 Controls UI로 못 고친다 → `control:false`로 숨기고 상태는 **별도 스토리**(예: Calendar `DisabledPast`)나 `mapping` 프리셋으로 보여준다. 스토리는 디자인 확인용이라 prop 타입을 1:1 미러링할 필요 없다. ↔ **제어 컴포넌트의 value/onChange는 `useArgs`로 묶어** 컨트롤이 실제로 먹게 한다(로컬 `useState`로 두면 컨트롤이 무시됨 — Calendar selected·AlertDialog open에서 겪음).
...
- **단일 선택은 Button 토글이 아니라 네이티브 radiogroup.** 상호배타 선택(세그먼트·카테고리 필터)은 `aria-pressed` 토글 버튼이 아니라 `fieldset`+native `radio`(pill 스타일)로 — 그룹 시맨틱·화살표 키 이동을 브라우저가 준다(faculty 정렬·공지 필터). 그 결과 Button `variant`(과거 `kind`에서 개명 — Tag와 prop명 통일, 업계 표준)는 **상태 없는 5개**(primary/neutral/secondary/quiet/nav)만(toggle·segmented 제거→radiogroup, 사용처 1곳이던 link는 Tag 내부 직접 정의). **아이콘은 `iconLeft` prop 없이 children에 직접**(shadcn식 — base의 `gap-2`가 간격 처리).
- **합의 대기(자율 실행 금지):** ① `#202020`(공지 필터 pill 비선택 배경) 신규 색 토큰 — 매칭 토큰 없어 디자인 결정. ② 패딩 **세로/컴포넌트 내부** 임의값 + `.62`/`.625` 근접중복 정규화(가로 거터는 완료).

---

**마지막 업데이트:** 2026-06-19 (① 테스트 인프라 정리: **CI 제거**(솔로 레포 → 로컬 `pnpm test` 단일 게이트), e2e-docker.sh가 백엔드를 항상 `../csereal-server-main`(main) compose로 보장, baseline 로컬 생성. ② Storybook **CSF Next** 마이그레이션(§④: `preview.meta`/`meta.story`·`defineMain`/`definePreview`·`fn()`·union prop args·모달 트리거+play·Chromatic 제거). 이전: 2026-06-16 마이그레이션·디자인 시스템 완료.)
