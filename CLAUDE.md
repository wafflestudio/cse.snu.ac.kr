...
- **브랜치:** `main`=production · `develop`=staging · `feature/*`·`fix/*`→`develop` PR · `hotfix/*`→`main` PR(후 develop back-merge). 직접 push 금지 — ruleset 이 PR 필수 + `web-test`·`api-test` 필수 체크 + force push 금지(admin 포함).
...
- **CI(`ci.yml`, PR):** 잡 둘. `api-test`(백엔드 소스의 내용 해시로 jar 를 캐시 — 히트면 Gradle 을 건너뛰고, 아니면 `gradlew build -x jar` 로 테스트와 bootJar 를 한 번에) · `web-test`(JS 워크스페이스 typecheck·lint·knip 뒤에 로컬과 같은 `e2e/run.sh`, api-test 의 jar 를 `JAR_STAGE=prebuilt` 로). E2E 는 항상 같은 커밋의 `apps/api` 로 돈다. **두 벌 관리 X — CI 는 로컬 스크립트·config 를 호출만 한다.** 두 잡 모두 항상 돌아 성공을 보고하므로 둘 다 필수 체크다(경로 필터로 건너뛰는 잡은 필수로 못 둔다 — "대기 중"으로 머지가 막힌다). PR 이 만든 캐시는 다른 PR 이 못 읽어 develop push 에서도 `api-test` 를 돌려 캐시를 채운다.
