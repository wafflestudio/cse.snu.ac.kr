...
`develop` → staging, `main` → production. 워크플로는 레포 루트 `.github/workflows/`(`ci.yml` 의 `api-test`, `deploy.yml`), 배포 스크립트·대상은 `infra/ops/host-deploy.sh`·`infra/production.env`·`infra/staging.env`.
