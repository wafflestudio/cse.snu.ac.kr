...
> 상태: **설계(구현했다가 보류)**. 이 설계대로 #8에서 구현했으나(nginx 엣지 + nonce
> 재주입 + 섹션 purge) 배포 전에 revert했다. 되살릴 땐 그 커밋을 참고.
>
> **왜 보류했나(2026-08-22):** prod가 마이그레이션 전 빌드에 머물러 있어 59커밋을 한 번에
> 올려야 하는 상황에서, 컨테이너 내부 토폴로지까지 nginx+Hono 2프로세스로 바꾸면 문제가
> 생겼을 때 앱 변경 탓인지 새 엣지 탓인지 분리되지 않는다. 게다가 **캐시 효율을 실측한 적이
> 없어** 이득이 미검증인 반면, CSP nonce가 entrypoint 랜덤비밀 → placeholder → nginx
> sub_filter 3단 간접을 타게 되어 교내 웹취약점 점검 대응 중에 CSP 표면을 넓히게 된다.
> 배포가 안정되면 staging에서 실측하고 다시 도입한다.
>
