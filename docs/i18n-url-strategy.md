...
> 결론: **모든 언어에 경로 프리픽스를 붙이고(`/ko/...`·`/en/...`), 프리픽스 없는 주소는 언어를 감지해 리다이렉트하는 전략(B)** 으로 전환한다.
>
> 상태: **전환 완료(2026-06-21)**. 당시엔 nginx ISR 캐싱의 선행 작업으로 착수했으나 **ISR은
> 이후 보류됐다**([[isr-csp-caching-plan]]). 아래 캐싱 관련 서술은 그 전제에서 쓰였다 —
> 다만 전환 자체의 근거(SEO·프레임워크 기본값·URL 안정성)는 캐싱과 무관하게 유효하다.
