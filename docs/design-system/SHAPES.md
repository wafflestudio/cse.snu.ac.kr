...
2026-09-24의 3-6 소스 조사와 이후 적용 기록을 함께 보관한다. **소스 조사 표만으로 새 형태 규칙 승인·앱 변경·렌더 검증 완료를 뜻하지 않는다.** 현재는 역할별 기존값 유지와 필요한 예외 적용을 정리하고 5단계 일반 시각 통합 검증을 진행 중이다. 남은 접근성 작업은 마지막에 재개한다.
...
| 4단계 판단 | 새 공통 크기·반경 토큰을 늘리지 않고 아래 역할별 기존값 유지. 작은 확인창의 실제 줄바꿈 문제는 DS-039로 예외 적용·대표 검증 |
...
## 4단계 후속 판단 — 기존 역할별 크기 유지

DS-016에 따라 반경·크기·그림자를 숫자 하나로 통일하지 않는다. 내용 Dialog의90vw/최대768px·90vh 내부스크롤, 짧은 확인 AlertDialog의최대512px·기존40/24px패딩, ImageModal의모바일최대320px/데스크톱400px·기존높이제한은 각각의 역할 기준으로 유지한다. 예약의 실제 넘침은 DS-026/029/030, ImageModal 상태는 DS-033/034로 다뤘다. 초기 조사의 미확정 형태값은 새 공통 크기 토큰을 정할 필요가 없는 유지 예외로 분류했다. 후속 예약 문구 비교에서 390px AlertDialog의 버튼 글자가 세로로 끊김을 확인해 DS-039로 640px 미만 90vw·최대512px, 버튼 글자 줄바꿈 금지·버튼 단위 줄바꿈을 적용했다. 640px 이상 내용 기반 폭과 기존 패딩은 유지한다. 버튼 줄바꿈으로 높이는 커질 수 있다. [예약 20개 배치](reservation-copy-implementation/measurements.json)와 [다른 확인창 4개](reservation-copy-implementation/other-alert-measurements.json)를 확인했다. 전체 읽기 화면의 일반 시각 검증은 5단계에서 진행 중이며 전체 통과를 뜻하지 않는다.
...
| 체크·라디오 | 체크 그림 18px·라벨 행 20px / 라디오 입력 14px | 그림만으로 전체 클릭 영역 적합성을 판단하지 않음. 체크 라벨·초점은 4단계에서 연결·대표 확인했고 남은 접근성 검증은 마지막에 진행 |
...
| 확인 AlertDialog | DS-039: 640px 미만 90vw, 그 이상 auto, 최대 32rem. 가로 40·세로 24px 패딩 유지 | 버튼 글자는 줄바꿈하지 않고 버튼 단위로 다음 줄 이동. 내용 Dialog와 목적이 다르며 별도 최대 높이·내부 스크롤 선언은 없음. 예약 한영 320/390/639/640/1200px와 다른 확인창 한영 320/1200px를 대표 확인했으며 모든 내용 길이를 검증한 것은 아님 |
...
## 3-6 당시 조사 기록 — 비교 후보와 대표 렌더 범위

아래 후보·미검증 표시는 당시 상태다. 이후 DS-026·029·030·033·034·039의 적용 결과는 각 문서와 위 현재 상태를 따른다. 현재 새 판단을 요청하는 목록이 아니다.
...
| 내용 Dialog·닫기 | [기존 교과목 상세](baseline/course-dialog-ko-390.png), [닫기 1280/390px 실측](icon-target-audit/measurements.json) | 기존 각진 상자와 24px 닫기 조작을 확인한 자료. 초기 기준은 후속 서체/색 변경 전이므로 최신 픽셀 기준으로 사용하지 않음. 이름 누락 Q-12는 이후 Dialog 닫기에 한영 이름을 연결해 대표 확인함 |
...
당시 최소 변경 후보는 **예약 전용 Fieldset에 `max-w-full`을 추가해 `w-88 max-w-full`로 두는 것**이다. 여유가 있을 때 352px를 유지하고 작은 화면에서만 부모 폭 이하로 제한하려는 후보였으며, 당시에는 실제 적용·승인안으로 확정하지 않았다. 이후 DS-026으로 승인·적용했다.
...
## 당시 후속 대표 확인 — 2026-09-24

위 소스 조사 후 예약 추가 Q-10을 실제 렌더로 확인했다. [S-Q1 전후 비교](reservation-fit.html)는 한·영 390/1280px와 모바일 날짜 펼침 10상태다. 390px에서 내용 폭295px/고정 필드352px의 넘침이 재현됐고, 로컬 최대폭 제한으로 필드295px·내부 scrollWidth351px가 됐다. 데스크톱 한·영 이미지는 전후 동일했다. **당시 임시 비교였으며 이후 DS-026으로 앱에 적용**했다.

AlertDialog·ImageModal의 역할별 크기·그림자 차이만으로 새 질문을 만들 근거는 없었다. 당시 예약 상세의 고정 최소폭320px는 실제 재현 전인 소스 후보였고, 이후 DS-030으로 적용·대표 검증했다. Dialog 닫기 이름은 한영에 연결했으며 남은 초점·복합 상태 검증은 마지막 접근성 범위다. 공지 빈 제출 오류 위치도 [검토 묶음](form-modal-review.html)에 함께 준비했다.

## DS-026 적용 후

예약 입력의 최대폭 제한은 승인·적용했다. [결과](RESERVATION-FIT.md). 작은 날짜 팝오버와 예약 상세는 별도 문제로 재현해 DS-029·030으로 [적용·대표 검증](reservation-boundary.html)했다. 이후 예약 용어·작은 확인창도 DS-038·039로 [적용·대표 검증](reservation-copy.html)했다. 기존 반경·그림자 값은 DS-016의 유지 기준에 따라 역할별 예외로 보존한다. 새로운 공통 반경/그림자 스케일을 추가하지 않는다.
