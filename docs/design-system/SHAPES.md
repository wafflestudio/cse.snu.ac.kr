...
| 4단계 판단 | 새 공통 크기·반경 토큰을 늘리지 않고 아래 역할별 기존값 유지. 작은 확인창의 실제 줄바꿈 문제만 RC-Q2 검토 중 |
...
예약 입력의 최대폭 제한은 승인·적용했다. [결과](RESERVATION-FIT.md). 작은 날짜 팝오버와 예약 상세는 별도 문제로 재현해 [다음 검토](reservation-boundary.html)에 분리했다. 기존 반경·그림자 값은 DS-016의 유지 기준에 따라 역할별 예외로 보존한다. 새로운 공통 반경/그림자 스케일을 추가하지 않는다.
