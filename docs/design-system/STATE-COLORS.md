# 오류 글자 사용 기준 — C-Q6 / 검색 초점 제안 철회

[현재 DS](index.html) · [견본과 실제 화면 비교](state-colors.html) · [진행 현황](PLAN.md)

**현재 상태: 검색 외곽선 제안은 DS-012로 철회했다. 오류 글자색 사용 기준만 판단 대기다.**
Text·TextArea에 이전에 추가했던 외곽선도 제거한다. 아래 검색 비교는 당시 제안의 근거를 보존한 것으로 현재 적용할 규칙이 아니다.
...
| 밝은 구역의 검색 초점 | 철회 · DS-012 | 기존 구현 유지. 새 외곽선 적용 안 함 |
| 어두운 헤더의 검색 초점 | 철회 · DS-012 | 기존 구현 유지. 새 외곽선 적용 안 함 |
...
검색 외곽선 제안은 DS-012로 철회했으므로 새 초점 규칙을 구현하지 않는다.
기본 입력 테두리·나머지 컨트롤 상태는 후속 비교에 남긴다. 전체 회귀와 접근성 검증을 대체하지 않는다.

## 이번 판단

**오류 글자는 흰색·neutral-50 바탕에서 기존 red-600으로 유지하는 기준으로 정할까?**
현재 값의 역할과 배경 범위를 정하는 질문이며, 검색 외곽선은 다시 승인받지 않는다.
