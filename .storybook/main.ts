...
export default defineMain({
  ...
  // `@/utils/serverFns`(createServerFn 기반)는 SB 브라우저 환경에서 `.validator` 체인이
  ...
  viteFinal: async (cfg) => {
    ...
    if (Array.isArray(alias)) {
      alias.unshift({ find: '@/utils/serverFns', replacement: serverFnsMock });
    } else {
      cfg.resolve.alias = {
        '@/utils/serverFns': serverFnsMock,
        ...(alias ?? {}),
      };
    }
